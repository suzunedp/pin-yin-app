...
## PWA インストールプロンプト ダイアログ

### フレーム情報

| 項目 | 値 |
|------|-----|
| フレーム名 | 青墨 Qīng Mò — 画像版 [PWAインストールプロンプト] |
| ベースフレーム | 青墨 Qīng Mò — 画像版 [手動で手直し2]（node-id: 26-2）のコピー |
| Figma node-id | 48-2 |
| フレームサイズ | iPhone 15 Pro (393×852px) |

### 概要

既存のメイン画面の上に、フル幅のボトムシート型PWAインストールプロンプトを画面最下部に表示するデザイン。背景暗転オーバーレイや drag-handle は**なし**。
...
```
Frame: 青墨 Qīng Mò — 画像版 [PWAインストールプロンプト] [w393 × h852]
  ├─ ... (ベースフレームのレイヤー: bg-base, image-layer, color-overlay, Content)
  └─ Frame: pwa-install-dialog  ← ボトムシート本体
      ├─ Frame: content-row         ← アイコン + テキスト横並び
      │   ├─ Frame: app-icon            ← アプリアイコン
      │   │   └─ Text: 拼
      │   └─ Frame: text-group          ← タイトル + 説明文
      │       ├─ Text: title
      │       └─ Text: description
      └─ Frame: button-section      ← ボタン群
          ├─ Frame: btn-install         ← プライマリボタン
          │   └─ Text
          └─ Frame: btn-dismiss         ← セカンダリボタン
              └─ Text
```
...
#### Frame: pwa-install-dialog
- 位置: 画面最下部（bottom=0、left=0、right=0）、フル幅
- Fill: Variable `bg/surface`（`rgba(255,255,255,0.8)`）
- backdrop-filter: blur(12px)
- 角丸: なし（全辺 0）
- Layout: Auto Layout 縦 (VERTICAL)
- Padding: top=32、bottom=32、left=20、right=20
- Gap: 20px

#### Frame: content-row（アイコン + テキスト）
- Layout: Auto Layout 横 (HORIZONTAL)
- Gap: 16px
- counterAxisAlignItems: CENTER（縦方向センタリング）
- Fill: なし
- Sizing: FILL × HUG

#### Frame: app-icon
- サイズ: w56 × h56（FIXED × FIXED）
- Fill: Variable `btn/primary-bg`（`rgba(46,76,94,0.9)`）
- Corner Radius: 12px
- Layout: Auto Layout 縦、primaryAxisAlignItems=CENTER、counterAxisAlignItems=CENTER

| レイヤー | テキスト | フォント | Fill Variable |
|---------|---------|---------|---------------|
| app-icon 内 Text | `拼` | Noto Serif SC Light 24px | `btn/primary-text` |

#### Frame: text-group
- Layout: Auto Layout 縦 (VERTICAL)
- Gap: 6px
- Fill: なし
- Sizing: FILL × HUG（flex: 1）

| レイヤー | テキスト | フォント | Fill Variable |
|---------|---------|---------|---------------|
| title | `ホーム画面に追加` | Noto Serif SC Light 17px、Letter Spacing 2% | `text/primary` |
| description | `アプリをホーム画面に追加すると、いつでも素早くアクセスできます` | Noto Sans SC Regular 12px、Line Height 160% | `text/secondary` |
...
**Frame: btn-install（プライマリ）**
- Fill: Variable `btn/primary-bg`
- Corner Radius: **6px**
- Padding: top=17、bottom=17、left=20、right=20
- primaryAxisAlignItems: CENTER
- Sizing: FILL × HUG

| レイヤー | テキスト | フォント | Fill Variable |
|---------|---------|---------|---------------|
| label | `ホーム画面に追加` | Noto Sans SC Regular **12px**、Letter Spacing 4% | `btn/primary-text` |

**Frame: btn-dismiss（セカンダリ）**
- Fill: Variable `bg/surface`
- Stroke: 0.5px、Variable `border/default`、INSIDE
- Corner Radius: **6px**
- Padding: top=17、bottom=17、left=20、right=20
- primaryAxisAlignItems: CENTER
- Sizing: FILL × HUG
...
| レイヤー | テキスト | フォント | Fill Variable |
|---------|---------|---------|---------------|
| label | `後で` | Noto Sans SC Regular **12px**、Letter Spacing 4% | `text/secondary` |

---

## PWA インストールプロンプト ダイアログ（iOS 版）

### フレーム情報

| 項目 | 値 |
|------|-----|
| フレーム名 | 青墨 Qīng Mò — 画像版 [PWAインストールプロンプト iOS] |
| ベースフレーム | 青墨 Qīng Mò — 画像版 [手動で手直し2]（node-id: 26-2）のコピー |
| Figma node-id | 53-2 |
| フレームサイズ | iPhone 15 Pro (393×852px) |
...
iOS Safari では `beforeinstallprompt` イベントが発火しないため、ユーザーが Safari のシェアシートから手動で追加する必要がある。そのため、Android 版のようなインストールボタンは持たず、操作手順を 2 ステップで案内するデザインになっている。
...
| 要素 | Android 版 | iOS 版 |
|------|-----------|--------|
| アプリアイコン | あり（拼 文字） | **あり**（Android と同仕様） |
| ボタン構成 | プライマリ（追加）＋セカンダリ（後で） | プライマリのみ（閉じる） |
| ステップ案内 | なし | あり（2 ステップ） |
...
```
Frame: 青墨 Qīng Mò — 画像版 [PWAインストールプロンプト iOS] [w393 × h852]
  ├─ ... (ベースフレームのレイヤー)
  └─ Frame: pwa-install-dialog-ios  ← ボトムシート本体
      ├─ Frame: content-row         ← アイコン + テキスト横並び（Android と同構造）
      │   ├─ Frame: app-icon
      │   │   └─ Text: 拼
      │   └─ Frame: text-group
      │       ├─ Text: title
      │       └─ Text: description
      ├─ Frame: steps-section       ← ステップリスト
      │   ├─ Frame: step-1-row      ← ① Safariシェアボタン手順
      │   │   ├─ Frame: step-num-1  ← 番号バッジ
      │   │   │   └─ Text: 1
      │   │   └─ Text: step-text
      │   └─ Frame: step-2-row      ← ② ホーム画面に追加手順
      │       ├─ Frame: step-num-2
      │       │   └─ Text: 2
      │       └─ Text: step-text
...
#### Frame: pwa-install-dialog-ios
- 位置: 画面最下部（bottom=0、left=0、right=0）、フル幅（Android と同仕様）
- Fill: Variable `bg/surface`（`rgba(255,255,255,0.8)`）
- backdrop-filter: blur(12px)
- 角丸: なし
- Layout: Auto Layout 縦 (VERTICAL)
...
#### Frame: content-row / app-icon / text-group
Android 版と同仕様。
...
**各 step-N-row（HORIZONTAL auto-layout）**
- Gap: 12px
...
**Frame: step-num-N（番号バッジ）**
- サイズ: w24 × h24（FIXED × FIXED）
- Fill: Variable `btn/primary-bg`
- Corner Radius: 12px（円形）
- primaryAxisAlignItems=CENTER、counterAxisAlignItems=CENTER

| レイヤー | テキスト | フォント | Fill Variable |
|---------|---------|---------|---------------|
| step-num 内 Text | `1` / `2` | Noto Sans SC Regular 11px | `btn/primary-text` |

**step テキスト**

| ステップ | テキスト内容 | フォント | Fill Variable |
|---------|------------|---------|---------------|
| step-1 | `Safariの下部にある共有ボタン（□↑）をタップ` | Noto Sans SC Regular 13px、Line Height 160% | `text/primary` |
| step-2 | `「ホーム画面に追加」を選択` | Noto Sans SC Regular 13px、Line Height 160% | `text/primary` |
...
| レイヤー | テキスト | フォント | Fill Variable |
|---------|---------|---------|---------------|
| label | `閉じる` | Noto Sans SC Regular **12px**、Letter Spacing 4% | `btn/primary-text` |
